...
function Bedroom() {
    ...
    useEffect(() => {
        const fetchDevices = async () => {
            try {
                const response = await fetch('http://localhost:8080/api/devices/bedroom'); // Use lowercase 'bedroom'
        ...
        try {
            ...
            if (ws && ws.readyState === WebSocket.OPEN) {
                ...
                const message = {
                    ...
                    room: 'bedroom',            // Room name (you can set it dynamically if needed)
                };
        ...
        if (normalizedNewDevice && allowedDevices.includes(normalizedNewDevice) && !devices.some((d) => d.name === normalizedNewDevice)) {
            try {
                const response = await fetch('http://localhost:8080/api/devices', {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ name: normalizedNewDevice, status: 'off', room: 'bedroom' }), // Use lowercase 'bedroom'
                });
                if (response.ok) {
